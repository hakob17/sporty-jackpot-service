...
The configs are polymorphic: `type` selects the shape, and each shape declares and validates exactly
its own fields in its compact constructor — so a `VARIABLE` contribution missing `poolStep` is a 400
naming the offending field, an out-of-range percentage is a 400, and `minPercentage` above
`percentage` is a 400. Because the request takes the config records directly, the same validation
runs whether a config arrives over HTTP, is read back out of the JSON column, or is built in a test.

Two cross-cutting rules that a single record cannot enforce on its own:

- A config is also checked against the jackpot it is attached to, via
  `RewardConfig.validateAgainstPool`. A `VARIABLE` reward whose `poolLimit` is at or below the
  jackpot's `initialPoolAmount` is rejected — its chance would be 100% from the start, and again
  after every payout, so every bet would win the whole pool. Overriding that hook is how a config
  opts in, which keeps it polymorphic instead of a branch on the config type.
- Unknown properties are **ignored**, not rejected — Spring's Jackson default. A `FIXED` config
  carrying a stray `poolStep` is accepted and the extra field is dropped. Setting
  `spring.jackson.deserialization.fail-on-unknown-properties: true` would tighten that, at the cost
  of rejecting clients that send extra fields.

Adding a configuration adds one record and one strategy; the existing ones are untouched.
