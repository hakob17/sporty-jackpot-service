...
public class Jackpot {
    ...
    public Jackpot(UUID id, String name, BigDecimal initialPoolAmount,
        ...
        this.rewardConfig.validateAgainstPool(this.initialPoolAmount);
    }
    ...
    public void reconfigure(ContributionConfig contributionConfig, RewardConfig rewardConfig) {
        ContributionConfig newContribution = requireConfig(contributionConfig, "contributionConfig");
        RewardConfig newReward = requireConfig(rewardConfig, "rewardConfig");
        // Checked against the initial amount, not the current one: that is what the pool returns to
        // after a payout, so a limit at or below it makes every future round win immediately.
        // Validate before assigning, so a rejected update leaves the jackpot exactly as it was.
        newReward.validateAgainstPool(this.initialPoolAmount);
        this.contributionConfig = newContribution;
        this.rewardConfig = newReward;
    }
