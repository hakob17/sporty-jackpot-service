...
| `dto` | Request/response/message payloads — records with `@Schema` docs; `Bet` is also the Kafka payload. `ContributionConfigRequest`/`RewardConfigRequest` are **sealed interfaces with one record per type**, so each shape validates exactly its own fields |
